...
export async function radial_areachart(
  ...
  // checkToken(token, "radial_areachart").then((isValid) => {
  //   if (!isValid) {
  //     return placeholder(div, size, colors, "radial_areachart");
  //   } else {
      if (file?.path) {
      ...
      const pivotedData: PivotedRow[] = Array.from(
        grouped,
        ([label, values]) => {
      ...
      const keys: string[] = Array.from(
        new Set(data.map((d: any) => d.category))
      );
      ...
      const area = horizontal
        ? d3
            ...
            .x0((d: any) =>
              (x as d3.ScaleLinear<number, number>)(d[0] as number)
            )
            .x1((d: any) =>
              (x as d3.ScaleLinear<number, number>)(d[1] as number)
            )
        ...
        : d3
            ...
            .x(xAccessor)
            .y0((d: any) =>
              (y as d3.ScaleLinear<number, number>)(d[0] as number)
            )
            .y1((d: any) =>
              (y as d3.ScaleLinear<number, number>)(d[1] as number)
            )
