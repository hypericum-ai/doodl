 ...
 *
 * @param {array} data Either the path to a csv file or inline data in doodl
 * @param {string} div The div in which the chart will be placed
 * @param {string} size The size (one of several preset sizes)
...
function animated_density(div, size) {
    ...
    var draw = function (processed_data, div, size) {
        ...
        var doodl_chart = new AnimatedDensity(processed_data, div, size, ["Random Variable with Gamma Distribution", "Occurrences"], scales);

        doodl_chart.add_svg().add_grid().add_density();
