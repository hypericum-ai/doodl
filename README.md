...
Glasseye was conceived with the goal of presenting the results of
...
glasseye offers a collection of customizable charts, with data
...
The charts included in glasseye are simple and uncluttered with the
...
Glasseye accepts data in JSON, CSV and TSV where appropriate, and
...
The charts currently included in glasseye include the following:
...
Because the Markdown input to glasseye is handed to pandoc to be
turned into HTML, glasseye can do anything that pandoc (with HTML
...
Adding Python-generated plots requires the `-p` argument to glasseye,
like this:

~~~
$ glasseye -p myfile.md
~~~

or

~~~
$ glasseye --plot myfile.md
~~~
...
~~~
$ glasseye -f pandoc-plot myfile.md
~~~

or

~~~
$ glasseye --filter pandoc-plot myfile.md
~~~

Any filter that you have installed for pandoc will work with
glasseye. Multiple filters can be provided by using the `-f` or
`--filter` arguments multiple times.

## How it works

Glasseye is built using pandoc and the python beautifulsoup
...
find the demo files in `path_to_your_packages/glasseye/demo`. Copy
...
See the
[demo](http://coppeliamla.github.io/glasseye/glasseye_markdownExample/demo.html)
